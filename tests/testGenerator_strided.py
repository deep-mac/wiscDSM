...
def putFunc(totalVars, totalOps, ratio, stride, fp):
    ...
    funcName = "test_" + str(totalVars) + "_" + str(totalOps) + "_" + str(ratio100) + "_strided_" + str(stride)
...
def putVars(totalVars, stride, fp):
    ...
    for i in range (totalVars):
        ...
        startAddr = startAddr + (0x1000 * stride)
    str_ = "\tint temp;"
    fp.write(str_)
    fp.write('\n')
...
fileName = ""
ratio100 = int(ratio*100)
fileName = "test_" + str(totalVars) + "_" + str(totalOps) + "_" + str(ratio100) + "_strided_" + str(stride) + ".h"
fp = open(fileName, "w")
